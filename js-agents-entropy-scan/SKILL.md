...
description: |
  JS/JSDoc 微内核项目代码降熵扫描与修复。适用于纯 JavaScript + JSDoc 架构的 Agent 系统。

  触发条件：
  - 用户请求对 js/agents 或类似 JS 微内核项目进行代码质量扫描
  - 用户说 "/js-entropy-scan" 或 "扫描代码熵"
  - 需要检查：JSDoc 覆盖率、TODO/FIXME 清理、console 残留、错误处理规范、导出一致性、死代码检测
---
