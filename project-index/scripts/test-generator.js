...
import { loadConfig, parseArgs, shouldProcess, readFileSafe, SAFETY_PROMPT_PREFIX } from './shared.js';
...
async function main() {
  ...
  await runner.run({
    ...
    buildPrompt: async function (item) {
      ...
      return `${SAFETY_PROMPT_PREFIX}你是一个测试工程师。请为以下源文件生成完整的单元测试。
...
只输出完整的测试文件代码，不要其他解释。代码必须以 \`\`\`javascript 开始，以 \`\`\` 结束。`;
    ...
    handleResult: async function (item, result) {
      // Normalize result to string - extract output from codeagent response
