 ...
 * Prevents codex from executing dangerous commands
 * @type {string}
 */
...
export function matchesPattern(relativePath, patterns) {
  ...
  /**
   * Convert a simple glob pattern into a regex source string.
   *
   * Supported:
   * - `*`  matches any chars except '/'
   * - `**` matches any chars including '/'
   * - `**` + `/` matches any dir prefix (including empty)
   *
    ...
    for (let i = 0; i < p.length; i++) {
      ...
      if (ch === '*') {
        ...
        if (next === '*') {
          // "**"
          i++;
          // Special-case "**/" to allow empty prefix.
          if (p[i + 1] === '/') {
            out += '(?:.*/)?';
            i++;
          } else {
  ...
  for (const pattern of patterns) {
    const regex = globToRegexSource(pattern);
