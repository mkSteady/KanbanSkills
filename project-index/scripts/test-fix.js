...
/**
 * Convert a path to posix separators for stable comparisons.
 * @param {string} p
 * @returns {string}
 */
...
/**
 * Load full cached test result file.
 * @param {string} cwd
 * @returns {Promise<any|null>}
 */
async function loadCachedResult(cwd) {
  const stateDir = path.join(cwd, '.project-index');
  const resultFile = path.join(stateDir, RESULT_FILE);
  return await readJsonSafe(resultFile, null);
}
...
async function loadCachedErrors(cwd, offset, limit) {
  const stateDir = path.join(cwd, '.project-index');
  const resultFile = path.join(stateDir, RESULT_FILE);
  const result = await loadCachedResult(cwd);
...
Usage: node test-fix.js [options] [sourceOrTestFile...]

Options:
  --dry-run        Preview fixes without applying
  --parallel       Treat positional args as a batch (alias; same as passing files)
  --concurrency=N  Parallel fix attempts (default 6)
  --offset=N       Start from error N (default 0)
  --limit=N        Process N errors (default 40)
  --resume         Resume from checkpoint
...
Filtering:
  - If you pass positional args, only cached errors whose sourceFile/testFile matches are processed.
    This is primarily used with test-prioritize.js output (root cause / batch / leaf files).
`);
    return;
  }
  ...
  const offset = Number.isFinite(args.offset) ? args.offset : (parseInt(args.offset) || 0);
  const limit = Number.isFinite(args.limit) ? args.limit : (parseInt(args.limit) || 40);

  const fileFilters = Array.isArray(args._)
    ? args._.map(normalizeRelInput).filter(p => p && p !== '.' && !p.startsWith('..'))
    : [];
  const hasFileFilters = fileFilters.length > 0;
   ...
   * @returns {boolean}
   */
  function matchesFilter(filter, candidate) {
    const f = normalizeRelInput(filter);
    const c = normalizeRelInput(candidate);
    if (!f || f === '.' || !c || c === '.') return false;

    // If filter includes path separators, treat as path match.
    if (f.includes('/')) return c === f;

    // Otherwise treat as basename match.
    return path.basename(c) === f;
  }
  ...
  if (hasFileFilters) {
    ...
    if (!result || !Array.isArray(result?.errors)) {
      log.error('No cached errors. Run: node scripts/test-result.js --save');
      process.exit(1);
    }
    const all = Array.isArray(result?.errors) ? result.errors : [];
    const failed = Number.isFinite(result?.failed) ? result.failed : all.length;

    const matching = all.filter(err => {
      const source = typeof err?.sourceFile === 'string' ? err.sourceFile : '';
      const testFile = typeof err?.testFile === 'string' ? err.testFile : '';
      return fileFilters.some(f => matchesFilter(f, source) || matchesFilter(f, testFile));
    });

    // By default, when filtering by files, process all matches unless user provided --limit.
    const effectiveLimit = args.limit === undefined ? Number.MAX_SAFE_INTEGER : limit;
    const slice = matching.slice(offset, offset + effectiveLimit);

    cached = {
      errors: slice,
      total: Number.isFinite(result?.total) ? result.total : failed,
      failed,
      passed: Number.isFinite(result?.passed) ? result.passed : 0,
      hasMore: offset + effectiveLimit < matching.length
    };

    if (cached.errors.length === 0) {
      log.error(`No cached errors matched filters: ${fileFilters.join(', ')}`);
      log.error('Hint: run `node scripts/test-result.js --save`, then rerun with correct file paths.');
      process.exit(1);
    }

    log.info(
      `Loaded ${cached.errors.length} matching errors (${offset + 1}-${offset + cached.errors.length} of ${matching.length}, total failing ${cached.failed})`
    );
  } else {
    cached = await loadCachedErrors(cwd, offset, limit);
  }
...
${err.received ? `- **Received**: ${err.received}` : ''}`).join('\n');
      ...
      return `${SAFETY_PROMPT_PREFIX}修复这个测试文件中的所有错误。
...
- **绝对禁止** 执行任何 Git 修改命令: git add, git commit, git checkout, git reset, git clean, git stash, git rm, git mv
- Git 只读命令允许: git status, git diff, git log, git show, git blame
- 只允许: 读取文件、修改文件、写入文件
- 测试结果已缓存，无需也不能验证修复
- 当前有多个并发任务，运行测试或 Git 修改命令会导致工作区损坏
...
- 失败: NOT_FIXABLE: <原因>`;
  ...
  if (cached.hasMore) {
    const suffix = hasFileFilters ? ` ${fileFilters.join(' ')}` : '';
    log.info(`\nMore errors available. Run: node test-fix.js --offset=${offset + limit}${suffix}`);
  }
}
