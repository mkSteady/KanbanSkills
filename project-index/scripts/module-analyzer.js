...
import { fileExists, loadConfig, parseArgs, readFileSafe, shouldProcess, SAFETY_PROMPT_PREFIX } from './shared.js';
...
async function main() {
  ...
  await runner.run({
    scan: async () => {
      // Build dependency map: parent depends on children (children must complete first)
      const allPaths = filtered.map(m => m.path);
      const pathSet = new Set(allPaths);

      return filtered.map(m => {
        // Find direct children of this module (subdirectories that are also in filtered)
        const dependencies = allPaths.filter(p =>
          p !== m.path &&
          p.startsWith(m.path + '/') &&
          // Only direct children (no intermediate paths in filtered)
          !allPaths.some(other =>
            other !== p &&
            other !== m.path &&
        ...
        return {
          id: m.path,
          modulePath: m.path,
          dependencies,  // Child modules must complete before parent
...
`;
      ...
      return `${SAFETY_PROMPT_PREFIX}你是一个代码分析专家。
